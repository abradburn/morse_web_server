 ...
 */
	class SpreadsheetReader_XLSX implements Iterator, Countable
	{
		 ...
		 */
		public function __construct(&$result = NULL, $Filepath, array $Options = null)
		{
			if(!is_null($result) || is_null($Filepath)){
				trigger_error('', E_USER_NOTICE);
				return FALSE;
			}

			// Temporary
			trigger_error("Opening file: {$Filepath}.", E_USER_NOTICE);

			if (!is_readable($Filepath))
			{
				//throw new Exception('SpreadsheetReader_XLSX: File not readable ('.$Filepath.')');
				trigger_error('SpreashseetReader_XLSX: File not readable ('.$Filepath.')');
				$result = FALSE;
				return FALSE;
			}
		...
		public function __destruct()
		{
			...
			{
				if(file_exists($this -> TempDir.'xl'.DIRECTORY_SEPARATOR.'worksheets')){
					@rmdir($this -> TempDir.'xl'.DIRECTORY_SEPARATOR.'worksheets');
				}

				if(file_exists($this -> TempDir.'xl')){
